...
def _get_model_parameters(model: GPTModel) -> Dict[str, np.ndarray]:
    ...
    for i, block in enumerate(model.transformer_blocks):
        ...
        parameters[f"block_{i}.attention.W_q.weight"] = (
            block.attention.W_q.weight.copy()
        )
        parameters[f"block_{i}.attention.W_q.bias"] = block.attention.W_q.bias.copy()
        parameters[f"block_{i}.attention.W_k.weight"] = (
            block.attention.W_k.weight.copy()
        )
        parameters[f"block_{i}.attention.W_k.bias"] = block.attention.W_k.bias.copy()
        parameters[f"block_{i}.attention.W_v.weight"] = (
            block.attention.W_v.weight.copy()
        )
        parameters[f"block_{i}.attention.W_v.bias"] = block.attention.W_v.bias.copy()
        parameters[f"block_{i}.attention.W_o.weight"] = (
            block.attention.W_o.weight.copy()
        )
        ...
        parameters[f"block_{i}.feed_forward.linear1.weight"] = (
            block.feed_forward.linear1.weight.copy()
        )
        parameters[f"block_{i}.feed_forward.linear1.bias"] = (
            block.feed_forward.linear1.bias.copy()
        )
        parameters[f"block_{i}.feed_forward.linear2.weight"] = (
            block.feed_forward.linear2.weight.copy()
        )
        parameters[f"block_{i}.feed_forward.linear2.bias"] = (
            block.feed_forward.linear2.bias.copy()
        )
...
def _set_model_parameters(model: GPTModel, parameters: Dict[str, np.ndarray]):
    ...
    for i, block in enumerate(model.transformer_blocks):
        # Attention parameters
        block.attention.W_q.weight = parameters[
            f"block_{i}.attention.W_q.weight"
        ].copy()
        block.attention.W_q.bias = parameters[f"block_{i}.attention.W_q.bias"].copy()
        block.attention.W_k.weight = parameters[
            f"block_{i}.attention.W_k.weight"
        ].copy()
        block.attention.W_k.bias = parameters[f"block_{i}.attention.W_k.bias"].copy()
        block.attention.W_v.weight = parameters[
            f"block_{i}.attention.W_v.weight"
        ].copy()
        block.attention.W_v.bias = parameters[f"block_{i}.attention.W_v.bias"].copy()
        block.attention.W_o.weight = parameters[
            f"block_{i}.attention.W_o.weight"
        ].copy()
        block.attention.W_o.bias = parameters[f"block_{i}.attention.W_o.bias"].copy()

        # Feed-forward parameters
        block.feed_forward.linear1.weight = parameters[
            f"block_{i}.feed_forward.linear1.weight"
        ].copy()
        block.feed_forward.linear1.bias = parameters[
            f"block_{i}.feed_forward.linear1.bias"
        ].copy()
        block.feed_forward.linear2.weight = parameters[
            f"block_{i}.feed_forward.linear2.weight"
        ].copy()
        block.feed_forward.linear2.bias = parameters[
            f"block_{i}.feed_forward.linear2.bias"
        ].copy()
