...
class DataLoader:
    ...
    def _load_and_tokenize_data(
        self, sample_texts: Optional[List[str]] = None
    ) -> np.ndarray:
        """
        ...
        for _ in range(self.batch_size):
            ...
            if max_start <= 0:
                ...
                repeated_data = np.tile(
                    self.token_data, (self.seq_length + 1) // len(self.token_data) + 1
                )
                start_idx = np.random.randint(
                    0, len(repeated_data) - self.seq_length - 1
                )
