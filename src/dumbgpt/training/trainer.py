...
class Trainer:
    ...
    def _get_model_parameters(self) -> dict:
        ...
        for i, block in enumerate(self.model.transformer_blocks):
            ...
            parameters[f'block_{i}.feed_forward.linear1.weight'] = block.feed_forward.linear1.weight
            parameters[f'block_{i}.feed_forward.linear1.bias'] = block.feed_forward.linear1.bias
            parameters[f'block_{i}.feed_forward.linear2.weight'] = block.feed_forward.linear2.weight
            parameters[f'block_{i}.feed_forward.linear2.bias'] = block.feed_forward.linear2.bias
    ...
    def train(self, num_steps: int, eval_interval: Optional[int] = None, 
              save_interval: Optional[int] = None, checkpoint_dir: Optional[str] = None) -> List[float]:
        """
        ...
        for step in range(num_steps):
            ...
            if eval_interval is not None and step % eval_interval == 0:
                ...
                print(f"Step {step}: Train Loss = {loss:.4f}, Eval Loss = {eval_loss:.4f}, Perplexity = {perplexity:.2f}")
            
            # Save checkpoint
            if save_interval is not None and checkpoint_dir is not None and step % save_interval == 0:
    ...
    def generate_samples(self, context_tokens: List[int], num_samples: int = 5, max_length: int = 50) -> List[List[int]]:
        """
